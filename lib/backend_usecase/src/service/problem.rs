...
use judge_core::{
    ...
    model::{dep_name_repository::DepNameRepository, problem_registry::ProblemRegistryServer},
};
...
impl<
    ...
    PRS: ProblemRegistryServer,
    DNR: DepNameRepository<ProblemId>,
> ProblemService<PR, UR, TR, PRC, PRS, DNR>
{
    pub fn new(
        problem_repository: PR,
        user_repository: UR,
...
> ProblemService<PR, UR, TR, PRC, PRS, DNR>
{
    ...
    pub async fn delete_problem(
        ...
        if session_user.is_none_or(|x| x.display_id != problem.author_id) {
            if problem.is_public {
                return Err(UsecaseError::Forbidden);
            } else {
                return Err(UsecaseError::NotFound);
