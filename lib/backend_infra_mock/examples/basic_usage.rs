...
async fn main() -> anyhow::Result<()> {
    ...
    for i in 1..=3 {
        ...
        mail_client
            .send_mail(addr, &format!("Test Subject {}", i), &format!("Test Body {}", i))
            .await?;
    ...
    for (idx, email) in sent_emails.iter().enumerate() {
        println!("  Email {}: To={}, Subject={}", idx + 1, email.to, email.subject);
    }
    ...
    auth_repo.save_user_password(user_id, "secure_password").await?;
    println!("✓ Password saved");
    
    let is_valid = auth_repo.verify_user_password(user_id, "secure_password").await?;
    println!("✓ Password verification: {}", is_valid);
    
    let is_invalid = auth_repo.verify_user_password(user_id, "wrong_password").await?;
    ...
    let google_oauth_id = auth_repo.get_google_oauth_by_authorize_code("auth_code_123", "login").await?;
    println!("✓ OAuth ID from code: {}", google_oauth_id);
    
    auth_repo.save_user_google_oauth(user_id, &google_oauth_id).await?;
    ...
    // GitHub OAuth
    println!("\n3. GitHub OAuth:");
    let github_oauth_id = auth_repo.get_github_oauth_by_authorize_code("github_code", "signup").await?;
    auth_repo.save_user_github_oauth(user_id, &github_oauth_id).await?;
    println!("✓ GitHub OAuth saved");
    
    // traQ OAuth
    println!("\n4. traQ OAuth:");
    auth_repo.save_user_traq_oauth(user_id, "traq_oauth_xyz").await?;
    ...
    let found_user = auth_repo.get_user_id_by_google_oauth(&google_oauth_id).await?;
    match found_user {
