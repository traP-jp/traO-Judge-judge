...
impl<
    ProblemRegistryClient: problem_registry::ProblemRegistryClient + Clone,
> job::JobApi<RegistrationToken, OutcomeToken> for JobApi<ProblemRegistryClient>
{
    async fn reserve_execution(
        ...
        Ok((0..count).map(|_| RegistrationToken { _marker: () }).collect())
    }
    ...
    async fn execute(
        ...
        let mut envvars = dependencies
            .iter()
            .map(|dep| (
                dep.envvar.clone(), dep.outcome.path().clone())
            )
            .collect::<HashMap<_, _>>();
        envvars.insert("THIS".to_string(), this.path().clone());
        std::fs::set_permissions(
            envvars.get("SCRIPT")
                .ok_or(ExecutionError::InternalError("No SCRIPT envvar".to_string()))?,
    ...
    async fn place_file(
        &self,
        file_conf: FileConf,
    ) -> Result<OutcomeToken, FilePlacementError> {
        let id = Uuid::new_v4();
        let path = self
            .temp_dir
            .join(id.to_string());
