...
pub fn common_module(parent: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // checker utilities
    sub_mod.add_function(pyo3::wrap_pyfunction!(checker::normal_judge_checker, &sub_mod)?)?;
    sub_mod.add_function(pyo3::wrap_pyfunction!(checker::parse_whitespace_and_newline, &sub_mod)?)?;
    Ok(())
}
