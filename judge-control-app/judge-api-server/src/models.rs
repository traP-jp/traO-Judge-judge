...
impl Execution {
    ...
    pub fn new(
        shell_script_id: uuid::Uuid,
        directory_count: f64,
        text_resource_count: f64,
        one_time_text_count: f64,
    ) -> Execution {
        Execution {
            optional_info: None,
            shell_script_id,
...
impl std::str::FromStr for Execution {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing Execution".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    "optionalInfo" => intermediate_rep.optional_info.push(
                        <models::OptionalInfo as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "shellScriptId" => intermediate_rep.shell_script_id.push(
                        <uuid::Uuid as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "directoryCount" => intermediate_rep.directory_count.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "textResourceCount" => intermediate_rep.text_resource_count.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "oneTimeTextCount" => intermediate_rep.one_time_text_count.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing Execution".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(Execution {
            ...
            shell_script_id: intermediate_rep
                .shell_script_id
                .into_iter()
                .next()
                .ok_or_else(|| "shellScriptId missing in Execution".to_string())?,
            directory_count: intermediate_rep
                .directory_count
                .into_iter()
                .next()
                .ok_or_else(|| "directoryCount missing in Execution".to_string())?,
            text_resource_count: intermediate_rep
                .text_resource_count
                .into_iter()
                .next()
                .ok_or_else(|| "textResourceCount missing in Execution".to_string())?,
            one_time_text_count: intermediate_rep
                .one_time_text_count
                .into_iter()
                .next()
                .ok_or_else(|| "oneTimeTextCount missing in Execution".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<Execution>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<Execution>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                "Invalid header value for Execution - value: {} is invalid {}",
                hdr_value, e
            )),
        }
    }
            ...
            std::result::Result::Ok(value) => {
                match <Execution as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        "Unable to convert header value '{}' into Execution - {}",
                        value, err
                    )),
                }
            }
...
impl std::fmt::Display for ExecutionConfigMap {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params: Vec<Option<String>> = vec![
            ...
            self.one_time_text_contents
                .as_ref()
                .map(|one_time_text_contents| {
                    [
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing ExecutionConfigMap".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                #[allow(clippy::match_single_binding)]
                match key {
                    "textResourceIds" => return std::result::Result::Err(
                        "Parsing a container in this style is not supported in ExecutionConfigMap"
                            .to_string(),
                    ),
                    "oneTimeTextContents" => return std::result::Result::Err(
                        "Parsing a container in this style is not supported in ExecutionConfigMap"
                            .to_string(),
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing ExecutionConfigMap".to_string(),
                        )
                    }
                }
            }
...
impl std::convert::TryFrom<header::IntoHeaderValue<ExecutionConfigMap>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<ExecutionConfigMap>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                "Invalid header value for ExecutionConfigMap - value: {} is invalid {}",
                hdr_value, e
            )),
        }
    }
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <ExecutionConfigMap as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        "Unable to convert header value '{}' into ExecutionConfigMap - {}",
                        value, err
                    )),
                }
            }
...
impl Judge {
    ...
    pub fn new(
        judge_id: uuid::Uuid,
        test_count: f64,
        before_test_execs: models::Execution,
        on_test_execs: models::Execution,
        after_test_execs: models::Execution,
        before_test_config_map: models::ExecutionConfigMap,
        on_test_config_maps: Vec<models::ExecutionConfigMap>,
        after_test_config_map: models::ExecutionConfigMap,
    ) -> Judge {
        Judge {
            judge_id,
            test_count,
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing Judge".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    #[allow(clippy::redundant_clone)]
                    "judgeId" => intermediate_rep.judge_id.push(
                        <uuid::Uuid as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "testCount" => intermediate_rep.test_count.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "beforeTestExecs" => intermediate_rep.before_test_execs.push(
                        <models::Execution as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "onTestExecs" => intermediate_rep.on_test_execs.push(
                        <models::Execution as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "afterTestExecs" => intermediate_rep.after_test_execs.push(
                        <models::Execution as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "beforeTestConfigMap" => intermediate_rep.before_test_config_map.push(
                        <models::ExecutionConfigMap as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    "onTestConfigMaps" => {
                        return std::result::Result::Err(
                            "Parsing a container in this style is not supported in Judge"
                                .to_string(),
                        )
                    }
                    #[allow(clippy::redundant_clone)]
                    "afterTestConfigMap" => intermediate_rep.after_test_config_map.push(
                        <models::ExecutionConfigMap as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing Judge".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(Judge {
            judge_id: intermediate_rep
                .judge_id
                .into_iter()
                .next()
                .ok_or_else(|| "judgeId missing in Judge".to_string())?,
            test_count: intermediate_rep
                .test_count
                .into_iter()
                .next()
                .ok_or_else(|| "testCount missing in Judge".to_string())?,
            before_test_execs: intermediate_rep
                .before_test_execs
                .into_iter()
                .next()
                .ok_or_else(|| "beforeTestExecs missing in Judge".to_string())?,
            on_test_execs: intermediate_rep
                .on_test_execs
                .into_iter()
                .next()
                .ok_or_else(|| "onTestExecs missing in Judge".to_string())?,
            after_test_execs: intermediate_rep
                .after_test_execs
                .into_iter()
                .next()
                .ok_or_else(|| "afterTestExecs missing in Judge".to_string())?,
            before_test_config_map: intermediate_rep
                .before_test_config_map
                .into_iter()
                .next()
                .ok_or_else(|| "beforeTestConfigMap missing in Judge".to_string())?,
            on_test_config_maps: intermediate_rep
                .on_test_config_maps
                .into_iter()
                .next()
                .ok_or_else(|| "onTestConfigMaps missing in Judge".to_string())?,
            after_test_config_map: intermediate_rep
                .after_test_config_map
                .into_iter()
                .next()
                .ok_or_else(|| "afterTestConfigMap missing in Judge".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<Judge>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<Judge>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                "Invalid header value for Judge - value: {} is invalid {}",
                hdr_value, e
            )),
        }
    }
...
impl std::convert::TryFrom<HeaderValue> for header::IntoHeaderValue<Judge> {
    ...
    fn try_from(hdr_value: HeaderValue) -> std::result::Result<Self, Self::Error> {
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => match <Judge as std::str::FromStr>::from_str(value) {
                std::result::Result::Ok(value) => {
                    std::result::Result::Ok(header::IntoHeaderValue(value))
                }
                std::result::Result::Err(err) => std::result::Result::Err(format!(
                    "Unable to convert header value '{}' into Judge - {}",
                    value, err
                )),
            },
            std::result::Result::Err(e) => std::result::Result::Err(format!(
...
impl std::fmt::Display for OptionalInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params: Vec<Option<String>> = vec![
            self.exec_time
                .as_ref()
                .map(|exec_time| ["execTime".to_string(), exec_time.to_string()].join(",")),
            self.memory_size
                .as_ref()
                .map(|memory_size| ["memorySize".to_string(), memory_size.to_string()].join(",")),
            self.language
                .as_ref()
                .map(|language| ["language".to_string(), language.to_string()].join(",")),
        ];
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing OptionalInfo".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    #[allow(clippy::redundant_clone)]
                    "execTime" => intermediate_rep.exec_time.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "memorySize" => intermediate_rep.memory_size.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing OptionalInfo".to_string(),
                        )
                    }
                }
            }
...
impl std::convert::TryFrom<header::IntoHeaderValue<OptionalInfo>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<OptionalInfo>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                "Invalid header value for OptionalInfo - value: {} is invalid {}",
                hdr_value, e
            )),
        }
    }
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <OptionalInfo as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        "Unable to convert header value '{}' into OptionalInfo - {}",
                        value, err
                    )),
                }
            }
